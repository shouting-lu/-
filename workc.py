...
def main(cfg, opt):
    ...
    ROI_ACTIONS_ID = [8, 11, 12, 14, 15, 37, 54, 57, 62, 63, 79]
    ...
    while(cap.isOpened()):
        ...
        for dets in preds:
            ...
            rec=[7,10,11,13,14,56,60,61,73,78]
            remo=[]
            ...
            if len(scores) > 0:
                ...
                for i, cls_ind in enumerate(indices):
                    ...
                    #text.append("[{:.2f}] ".format(scores[i]) + str(labelmap[cls_ind]['name']))
                ...
                state=''
                if 11 in indices or 13 in indices or 7 in indices or 56 in indices:
                    state = 'Off Duty'
                    cv2.rectangle(frame, (x1,y2-16), (x1+80,y2), (0,255,0), 2)
                    cv2.putText(frame, state, (x1+3,y2-5), font, 0.5, (255, 0, 0), 1)
                else:
        ...
        if vid_writer is None:
            save_path = os.path.join(opt.save_path, 'tg1-output.mp4')
